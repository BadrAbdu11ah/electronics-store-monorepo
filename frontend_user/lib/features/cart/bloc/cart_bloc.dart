...
class CartBloc extends Bloc<CartEvent, CartState> {
  ...
  Future<void> _onLoadCart(_LoadCart event, Emitter<CartState> emit) async {
    emit(
      state.copyWith(
        status: const CartStatus.loading(),
    ...
    response.fold(
      ...
      (cartResponse) {
        ...
        // قراءة الكوبون المفعّل مسبقاً في السلة إن وجد
        final activeCoupon = cartResponse.coupon;

        emit(
          state.copyWith(
            status: const CartStatus.loaded(),
            cartItems: cartResponse.items,
            subtotalPrice: cartResponse.subtotal ?? 0.0,
            shippingPrice: cartResponse.shippingPrice ?? 0.0,
            totalAppPrice: cartResponse.totalPrice ?? 0.0,
            totalQuantity: cartResponse.totalQuantity ?? 0,
            // تحديث حالة الكوبون تلقائياً في الشاشة عند الفتح
            couponId: activeCoupon?.id?.toString(),
            couponName: activeCoupon?.name,
            discountPercentage: activeCoupon?.discount?.toInt() ?? 0,
          ),
        );
      },
    );
  ...
  Future<void> _onDeleteItem(_DeleteItem event, Emitter<CartState> emit) async {
    // 1. تصفية القائمة محلياً لحذف المنتج تماماً بأمان
    ...
    // 3. تحديث الـ State فوراً بالأسعار الجديدة
    emit(
      state.copyWith(
        couponStatus: const CouponStatus.initial(),
  ...
  Future<void> _onApplyCoupon(
    ...
    response.fold(
      ...
      (couponModel) {
        final int discount = couponModel.discount?.toInt() ?? 0;
